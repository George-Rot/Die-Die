...
@onready var paredeInv = $Walls/wallBoss
...
func check_map_completion():	
	if GameManager.layer == 20 and paredeInv:
		paredeInv.queue_free()
		paredeInv = null
		
	
	if GameManager.killCount >= 2:
		GameManager.layer += 1
		GameManager.killCount = 0
		print(GameManager.totalKill)
